...
import GNOSIS_ICON from '../assets/wallets/trustwallet-icon.svg'
import WALLET_CONNECT_ICON from '../assets/wallets/walletconnect-icon.png'
...
const eip6963Connection: InjectedConnection = {
...
const [web3Network, web3NetworkHooks] = initializeConnector<Network>(
  (actions) => new Network({ actions, urlMap: RPC_PROVIDERS, defaultChainId: SupportedChainId.XFI_TESTNET })
)
...
const [deprecatedWeb3Network, deprecatedWeb3NetworkHooks] = initializeConnector<Network>(
  (actions) =>
    new Network({
      ...
      defaultChainId: SupportedChainId.XFI_TESTNET,
    })
)
...
const deprecatedInjectedConnection: Connection = {
  getProviderInfo: (isDarkMode: boolean) => getDeprecatedInjection(isDarkMode) ?? { name: `Browser Wallet` },
  connector: web3Injected,
  hooks: web3InjectedHooks,
  type: ConnectionType.INJECTED,
  shouldDisplay: () => true, //getIsMetaMaskWallet() || getShouldAdvertiseMetaMask() || getIsGenericInjector(),
...
export const walletConnectV2Connection: Connection = new (class implements Connection {
  private initializer = (actions: Actions, defaultChainId = SupportedChainId.XFI_TESTNET) =>
